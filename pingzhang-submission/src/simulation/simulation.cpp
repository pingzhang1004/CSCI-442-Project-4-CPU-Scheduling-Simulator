...
Simulation::Simulation(FlagOptions flags) {
    ...
    // TODO: Add your other algorithms as you make them
        ...
        if (flags.time_slice > 0){
            this->scheduler = std::make_shared<RRScheduler>(RRScheduler(flags.time_slice));   
        }
        else{
...
SystemStats Simulation::calculate_statistics() {
    /*
        TODO: Calculate the following system statistics:
            ...
            - cpu_efficiency  
    */ 
    std::shared_ptr<Process> proess_ptr;
    for (const auto& process : processes) {    
        //set the copy oof second object to the process
        proess_ptr = process.second;
        for (const auto& thread : proess_ptr->threads) {   
            //thread_counts[4]
            system_stats.thread_counts[proess_ptr->priority]++;
           //total_io_time
           system_stats.total_io_time += thread->io_time;
           //total_service_time
           system_stats.total_service_time += thread->service_time;
            //total thread_response_times[4]
            system_stats.avg_thread_response_times[proess_ptr->priority] += (double)thread->response_time();
           //total thread_turnaround_times[4]
            system_stats.avg_thread_turnaround_times[proess_ptr->priority] += (double)thread->turnaround_time();
        }
    }
    
    for (size_t i = 0; i < 4; i++)
    {
        //thread_acounts[4] represent the count of threads of the different priorities.  
        if (system_stats.thread_counts[i] != 0) {
        //average thread_response_times[4]
        system_stats.avg_thread_response_times[i] /= system_stats.thread_counts[i];
        //average thread_turnaround_times[4]
        system_stats.avg_thread_turnaround_times[i] /= system_stats.thread_counts[i];
        }
    }

    //total_cpu_time = service_time + dispatch_time.
    //total_idle_time = total_time - service_time - dispatch_time.
    system_stats.total_idle_time = system_stats.total_time - (system_stats.total_service_time + system_stats.dispatch_time);
    //cpu_utilization
    system_stats.cpu_utilization = ((double)(system_stats.total_service_time + system_stats.dispatch_time) / system_stats.total_time)* 100 ;
    //cpu_efficiency
    system_stats.cpu_efficiency = ((double)system_stats.total_service_time / system_stats.total_time)* 100;
